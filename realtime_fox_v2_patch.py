...
# ============================================================
# 1. Backend: Realtime API with optional session context
# ============================================================

Path("backend/app/api/realtime.py").write_text(
r'''import json
import os
...
if import_line not in text:
    ...
    for index, line in enumerate(lines):
        if (
            line.startswith("from app.api.")
            or line.startswith("from fastapi")
...
if include_line not in text:
    ...
    if matches:
        ...
        text = text[:insert_at] + include_line + text[insert_at:]
    else:
        text = text.rstrip() + "\\n\\n" + include_line + "\\n"
...
# ============================================================
# 3. Frontend launcher button
# ============================================================

launcher = Path("frontend/src/components/RealtimeFoxLauncher.jsx")
...
import_component = 'import RealtimeFoxLauncher from "./components/RealtimeFoxLauncher";\n'

if import_component not in app_text:
    marker = 'import VoiceAssistant from "./components/VoiceAssistant";\n'

    if marker in app_text:
        app_text = app_text.replace(marker, marker + import_component, 1)
    else:
        ...
        lines.insert(insert_at, import_component)
        app_text = "".join(lines)

if "<RealtimeFoxLauncher />" not in app_text:
    markers = [
        '<section className="workspace-card ask-card">',
    ...
    if not inserted:
        if "</main>" in app_text:
            ...
            print("WARNING: Could not auto-insert RealtimeFoxLauncher in App.jsx")
...
if "/* Realtime Fox launcher */" not in css_text:
    css_text += extra_css

css.write_text(css_text, encoding="utf-8")


print("Realtime Fox v2 patch applied.")
